...
def image_model(inputs: dict) -> str | list[str] | dict:
    ...
    model = ChatGoogleGenerativeAI(api_key=os.environ["GEMINI_API_KEY"], model="gemini-1.5-flash", temperature=0.3)
    ...
    Your role is to meticulously analyze the provided prescription images and extract all relevant information with the highest degree of precision.
    ...
    For each medication, also provide a structured "advice" section with:
   - Common side effects
   - Crucial precautions
   - Potential drug interactions
   - General health advice for the patient    

    Summarize the overall prescription in a concise paragraph.  
   - What is the prescription mainly for?  
   - General observations about the treatment.   
   - Any important instructions for the patient.
                    ...
                    medications_df = pd.DataFrame([
                        {
                            ...
                            "Side Effects": med["advice"]["common_side_effects"],
                            "Precautions": med["advice"]["precautions"],
                            "Drug Interactions": med["advice"]["drug_interactions"],
                            "General Advice": med["advice"]["general_advice"],
                        }
